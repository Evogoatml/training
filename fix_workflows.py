#!/usr/bin/env python3
"""Regenerate workflow JSONs with proper parent/child boundaries."""
...
EXCLUDE_DIRS = {".git", "venv", "__pycache__", ".idea", ".vscode",
                "node_modules", ".backend", "workflow", "bin", "obj", "Debug", "Properties"}
...
for d in all_dirs:
    if d == BASE_DIR:
        continue
    # Skip if under an agent root
    if any(d.is_relative_to(a) and d != a for a in agent_dirs):
        continue
    # Must have code files directly inside
    try:
        if any(f.is_file() and f.name.endswith((".py", ".ipynb")) for f in d.iterdir()):
            candidate_dirs.append(d)
    except OSError:
        pass

# Deepest-root-wins: sort by depth descending
...
# ------------------------------------------------------------------
# Assign every file to deepest project root
# ------------------------------------------------------------------
file_to_project = {}
orphan_files = []
...
def build_workflow(project_dir, files, agent=None, is_synthetic=False):
    ...
    for fp in sorted(files):
        ...
        nodes.append({
            ...
            "label": fp.name,
            "file_path": rel,
            "metadata": {"size_bytes": size, "extension": ext, "role": role}
        })
